...
def get_args():
    ...
    arg("-c", "--config_path", default='config/Urban/unetformer.py',type=Path, help="Path to the config.")
    parser.add_argument("-s","--strategy",default='auto',type=str)
...
def main():
    ...
    trainer = pl.Trainer(devices=config.gpus, max_epochs=config.max_epoch, accelerator='auto',
                         ...
                         callbacks=[checkpoint_callback,early_stop_callback], strategy=args.strategy,
                         logger=logger)
