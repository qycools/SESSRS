...
def get_sam_label(model, image,box_nms =0.7,min_mask_region_area=10, pred_iou_thresh=0.88,stability_score_thresh=0.92):
    
    size = image.size
    image_array = np.array(image)
    # image_array_copy = torch.from_numpy(image_array.copy()).permute(2,0,1).cuda()
    ...
    mask = mask_generator.generate(image_array)

    label_RGB,label_P = show_demo(size,mask)
    label = Image.fromarray(label_RGB.astype('uint8')).convert('RGB')
    blend = Image.blend(image,label,0.25)
    label_P = label_P.astype(np.uint16)
    return label,blend,label_P

def get_sam_json(model, image,box_nms =0.7,min_mask_region_area=10, pred_iou_thresh=0.88,stability_score_thresh=0.92):
    ...
    data = mask_generator.generate2(image_array)
...
def show_demo2(size,anns):
    ...
    for ann in sorted_anns:
        ...
        flag = flag+1
        demo = Image.fromarray(images)
        demo.save(f'demo/images_3_{flag-2}.png')
    # unique_elements = unique3D(images)
    # images2 = get_category(images,unique_elements)
