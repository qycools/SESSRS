...
class GeneralizedMaskDINO(nn.Module):
    ...
    def evaluate_demo(self, batched_inputs,all_whole=None,all_part=None,mask_features=None,multi_scale_features=None,return_features=False, level=[0,1,2,3,4,5]):
        ...
        # self.sem_seg_head.pixel_decoder == MaskDINOEncoder()
        # features == D2SwinTransformer
        ...
        height  = image_size[0]
        width   = image_size[1]
        # 对pred_masks中的C*W*H中的C（200）不是很理解，如果是number of classes,那么pred_ious 中的200是什么意思呢
        # 这里的C是batch point 的数量，每个bactch points的数量为200，不是number of classes
        
        if self.sem_seg_postprocess_before_inference:
            # retry_if_cuda_oom这个应该是防止爆内存的一种做法
